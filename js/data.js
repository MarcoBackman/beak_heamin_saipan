...
window.SAIPAN = (function(){
  const SPOTS = [
    { island:"saipan", nm:"그로토", en:"The Grotto", kind:"프리다이빙", ll:[15.2604,145.8239],
      imgs:["./img/hero-grotto.jpg","./img/day2-4.jpg","./img/day2-5.jpg","./img/day2-6.jpg"],
  ...
  /* 편의 지점 — 장비 대여소·맛집·상점. 좌표는 OSM(Nominatim/Overpass)으로 검증. cat: rental | food | shop */
  const POIS = [
    { cat:"rental", nm:"다이브위시", en:"Divewish", ll:[15.2127,145.7181], url:"https://www.padi.com/dive-center/northern-mariana-islands/divewish/",
      note:"가라판 Route 308 · 한국어·프리다이빙 장비 대여. 지도 좌표는 도로 기준 근사치이므로 예약 때 정확한 입구 확인" },
    { cat:"rental", nm:"아쿠아 커넥션", en:"Aqua Connections", ll:[15.2130,145.7190], url:"https://saipan-aquaconnections.com/",
      note:"가라판 로열팜 애비뉴 · 미크로네시아 최대 다이빙 장비점. 마스크·핀·슈트·웨이트 등 렌탈+판매" },
    { cat:"rental", nm:"마사 다이브", en:"Masa Dive Saipan", ll:[15.2131,145.7175], url:"https://www.masadive-saipan.com/",
      note:"가라판 진저 스트리트 · 장비 렌탈 + 보트 스노클. ☎ 670-233-0577" },
    { cat:"rental", nm:"메이 다이브 1986", en:"Saipan Mei Dive", ll:[15.2132,145.7174],
      note:"가라판 진저 스트리트 · 1986년부터 운영한 노포 다이브샵. 장비 렌탈·투어" },
    { cat:"rental", nm:"다이브 사이판", en:"Dive Saipan", ll:[15.2052,145.7169],
      note:"가라판 남단 · 비치·보트 다이빙 전문 다이브샵. 장비 렌탈 문의" },
    { cat:"food", nm:"컨트리 하우스", en:"Country House", ll:[15.2128,145.7182],
      note:"가라판 · 사이판 대표 스테이크·바비큐 (가이드 추천 맛집)" },
    { cat:"food", nm:"하드록 카페", en:"Hard Rock Cafe", ll:[15.2087,145.7176],
      note:"비치로드 · 버거·립·라이브 음악 랜드마크" },
    { cat:"food", nm:"360 회전 레스토랑", en:"Revolving Restaurant 360", ll:[15.1563,145.7061],
      note:"수수페 · 사이판 유일 회전 전망 레스토랑 (야경 디너)" },
    { cat:"food", nm:"윈첼스 도넛", en:"Winchell's Donuts", ll:[15.2129,145.7167],
      note:"가라판 · 24시간 도넛·현지식 아침" },
    { cat:"shop", nm:"DFS 갤러리아", en:"DFS Galleria", ll:[15.2102,145.7181],
      note:"비치로드 · 명품·화장품 면세 쇼핑 랜드마크" },
    { cat:"shop", nm:"아이러브사이판", en:"I Love Saipan", ll:[15.2130,145.7196],
      note:"가라판 · 기념품·간식 원스톱 (Day 7 쇼핑 코스)" },
    { cat:"shop", nm:"ABC 스토어", en:"ABC Stores", ll:[15.2143,145.7182],
      note:"가라판 · 편의·기념품 체인 (간식·선크림·물·기념품)" },
    { cat:"shop", nm:"조텐 슈퍼스토어", en:"Joeten Super Store", ll:[15.1717,145.7137],
      note:"찰란피아오 · 현지 최대 대형 슈퍼마켓 (생필품·물·주류)" },
    { cat:"shop", nm:"와일드웨스트 기념품", en:"Wild West Gift Shop", ll:[15.2129,145.7185],
      note:"가라판 · 티셔츠·기념품 선물 숍" },
    { cat:"shop", nm:"아쿠아스미스", en:"AQUASMITH", ll:[15.2077,145.7184],
      note:"가라판 · 다이빙·수상 레저 용품 전문 숍" },
    { cat:"stay", nm:"켄싱턴 호텔", en:"Kensington Hotel", ll:[15.2525,145.7776],
      note:"산로케 · 7박 연박 베이스 (전 객실 오션뷰·인피니티 풀·골프 패키지)" },
    { cat:"stay", nm:"Coral Garden Hotel", en:"Coral Garden Hotel / Rota", ll:[14.1382,145.1405],
      note:"예약 확정 · 9/12 13:00 체크인, 9/13 07:30~12:00 체크아웃" },
    { cat:"place", nm:"산호세", en:"San Jose", ll:[15.1659,145.7080],
      note:"사이판 남서부 마을 (오레아이·찰란카노아 인근)" },
  ];
  ...
  const GEAR_VENDOR_COORDS = {
    default: [15.2130,145.7190],             // 미선택 시 가라판 대여점 밀집 지역
    divewish:[15.2127,145.7181],             // Route 308 주소 기준 근사치 — 예약 시 입구 확인
    aqua:    [15.2130,145.7190],
    masa:    [15.2131,145.7175],
  };
  const GEAR_SHOP   = GEAR_VENDOR_COORDS.default;
  const SMILING_COVE= [15.2170, 145.7244];   // 스마일링 코브 마리나
  const MANAGAHA    = [15.2414, 145.7127];   // 마나가하 섬
  const MOBIL_GARAPAN=[15.2049,145.7170];    // Mobil Garapan (OSM)
  const ROTA_AIRPORT= [14.1743, 145.2425];   // 로타 공항
  const CORAL_GARDEN= [14.1367, 145.1408];   // Coral Garden Hotel 인근 도로 경로 앵커
  ...
  const DAYS = [
    ...
    { label:'Day 2 · 9/11 금', title:'렌터카 인수 · 장비 대여 · 프리다이빙',
      stops:[
        {n:'Enterprise 렌터카 인수 (02:30)', ll:SPN_AIRPORT},
        {n:'켄싱턴 호텔 체크인', ll:KENSINGTON, mode:'drive'},
        {n:'선택한 장비점 · 풀세트 인수', ll:GEAR_SHOP, mode:'drive', gearVendorStop:true},
        {n:'라우라우 베이 🐢', ll:LAULAU, mode:'drive'},
        {n:'그로토 (빛기둥 프리다이빙)', ll:GROTTO, mode:'drive'},
        {n:'파우파우 비치 몸풀기·회복', ll:PAUPAU, mode:'drive'},
        {n:'켄싱턴 호텔 복귀', ll:KENSINGTON, mode:'drive'},
      ]},
    { label:'Day 3 · 9/12 토', title:'로타 이동 · 로타 홀 빛내림',
      stops:[
        {n:'켄싱턴 호텔 (연박 — 짐은 방에)', ll:KENSINGTON},
        {n:'사이판 공항 · 09:30 체크인 (S2 1302)', ll:SPN_AIRPORT, mode:'drive'},
        {n:'로타 공항 · 11:00 도착', ll:ROTA_AIRPORT, mode:'fly', min:30},
        {n:'Coral Garden Hotel · 12시 전후 도착', ll:CORAL_GARDEN, mode:'drive'},
        {n:'로타 홀 빛내림 ✨ (보트)', ll:ROTA_HOLE, mode:'boat', min:20},
        {n:'피나탕 파크', ll:PINATANG, mode:'drive'},
      ]},
    { label:'Day 4 · 9/13 일', title:'테테토 · 스위밍 홀 → 복귀',
      stops:[
        {n:'Coral Garden Hotel · 08:00 체크아웃', ll:CORAL_GARDEN},
        {n:'테테토 비치 아침 수영', ll:TETETO, mode:'drive'},
        {n:'스위밍 홀 (천연 풀)', ll:SWIM_HOLE, mode:'drive'},
        {n:'로타 공항 · 15:00 체크인 (S2 3104)', ll:ROTA_AIRPORT, mode:'drive'},
        {n:'사이판 공항 · 17:00 도착, 렌터카 회수', ll:SPN_AIRPORT, mode:'fly', min:30},
        {n:'켄싱턴 호텔 (인피니티 풀 선셋)', ll:KENSINGTON, mode:'drive'},
      ]},
    { label:'Day 5 · 9/14 월', title:'마나가하 섬 · 숏핀 스노클 🏝️',
      stops:[
        {n:'켄싱턴 호텔', ll:KENSINGTON},
        {n:'스마일링 코브 마리나 · 주차', ll:SMILING_COVE, mode:'drive'},
        {n:'마나가하 섬 · 숏핀만', ll:MANAGAHA, mode:'boat', min:20},
        {n:'스마일링 코브 마리나 복귀', ll:SMILING_COVE, mode:'boat', min:20},
    ...
    { label:'Day 7 · 9/16 수', title:'장비 반납 · 가라판 마지막 밤',
      stops:[
        {n:'켄싱턴 리조트 모닝 (인피니티 풀 · 카약)', ll:KENSINGTON},
        {n:'선택한 장비점 · 대여 장비 반납', ll:GEAR_SHOP, mode:'drive', gearVendorStop:true},
        {n:'아이러브사이판 쇼핑 🛍️', ll:ILOVESAIPAN, mode:'drive'},
        {n:'Mobil Garapan · 반납 전 만땅 주유', ll:MOBIL_GARAPAN, mode:'drive'},
        {n:'가라판 디너 🥂', ll:GARAPAN, mode:'drive'},
        {n:'켄싱턴 호텔 · 짐 최종 정리', ll:KENSINGTON, mode:'drive'},
      ]},
    { label:'Day 8 · 9/17 목', title:'새벽 비행기로 귀국',
      stops:[
        {n:'켄싱턴 호텔 체크아웃', ll:KENSINGTON},
        {n:'Enterprise 반납 (예약 02:30 · 조기 반납 협의)', ll:SPN_AIRPORT, mode:'drive'},
  ...
  const DETAILS = {
    1: {
      todo: [
        { t:'오전',  x:'여권·E-티켓·달러 현금·Enterprise 예약 내용을 오프라인 저장하고 체크리스트 완료' },
        { t:'오전',  x:'본인 SKT T 괌·사이판 5천원, 아내 KT M모바일 함께 쓰는 로밍 4GB 신청·데이터 로밍 설정 확인' },
      ...
      bring: [
        ...
        '달러 현금 — 소액권 위주 (팁·입장료)', '주운전자 명의 실물 신용카드 — 예상 결제액 $268.05 + 보증금 $200',
        '두 사람 실물 운전면허증·국제운전면허증·여권 — 디지털 면허증 불가',
        '롱핀·숏핀·마스크·스노클 — 나머지 프리다이빙 풀세트는 현지 {gearDays} 대여', '래시가드·수영복·아쿠아슈즈',
      ...
      tips: [
        ...
        'SKT/KT M모바일 로밍을 출국 전에 각각 신청하고, DOCOMO $25 Visitor SIM은 현지 장애 때만 백업으로 사용',
        '도착이 새벽 2시 — 기내에서 자 둬야 다음 날 다이빙 컨디션이 산다',
      ],
    },
    2: {
      todo: [
        { t:'02:00', x:'사이판 입국 심사 → 수하물 수령 후 Enterprise 공항 카운터 이동' },
        { t:'02:30', x:'실물 면허증·국제면허증·여권·주운전자 카드 제시, 차량 상태 촬영 후 SUV 인수' },
        { t:'03:05', x:'공항 출발 → 켄싱턴까지 야간 운전 약 30~35분, 체크인 후 수면' },
        { t:'08:10', x:'조식 후 차량으로 가라판 이동 — {gearShop} 영업·피팅 예약 재확인' },
        { t:'08:40', x:'{gearShop}에서 핀·스노클 제외 풀세트 피팅 후 {gearDays} 대여 (D2 인수→D7 반납)' },
        { t:'09:40', x:'라우라우 베이에서 얕게 몸풀기·이퀄라이징 점검 후 가이드 판단에 따라 입수' },
        { t:'11:30', x:'그로토 가이드 투어 — 빛기둥 시간과 적기(빨간 깃발) 여부 확인' },
        { t:'14:30', x:'파우파우 비치에서 회복 수영 후 켄싱턴 복귀' },
        { t:'저녁',  x:'장비 민물 헹굼·건조 + 내일 로타행 짐 20lb 분리 패킹' },
      ],
      bring: [
        'Enterprise 예약 화면·실물 면허증·국제면허증·여권·주운전자 카드',
        '개인 마스크·스노클·롱핀', '두 사람 키·체중·발/슈트 사이즈 메모', '리프부츠 (라우라우 필수)',
        '아쿠아슈즈 (그로토 계단 112개)', '래시가드 또는 1.5~3mm 슈트', '방수백 (스콜 대비)',
        '그로토 입장료 $5 현금', '물·간식', '리프 세이프 선크림', '수건 2장',
        '귀중품은 호텔에 (라우라우 차량털이 이력)',
      ],
      tips: [
        '차량 인수 때 네 면·휠·유리·연료 게이지를 동영상으로 남기고 기존 흠집을 계약서에 표시',
        '장비점에서 슈트·마스크·고무벨트·웨이트·부이·랜야드·라인커터·컴퓨터 포함 여부와 보증금 확인',
    ...
    3: {
      todo: [
        { t:'08:00', x:'조식 후 로타 짐 최종 계량 — 위탁 20lb+개인 소지품 10lb, 캐리어·롱핀·웨이트·부이는 방에 그대로' },
        { t:'08:45', x:'렌터카로 사이판 공항 이동 (약 30~35분), 귀중품 없이 공영주차장에 1박 주차' },
        { t:'09:30', x:'커뮤터 터미널 체크인·수하물 계량 — 공식 마감보다 45분 더 여유 확보' },
        { t:'10:30', x:'스타 마리아나스 S2 1302 출발 → 11:00 로타 도착' },
        { t:'11:00', x:'수하물 수령·예약한 렌터카 인수·차량 촬영 후 11:40 전후 공항 출발' },
        { t:'11:50–12:20', x:'Coral Garden Hotel 도착 — 전달한 12~13시 범위, 객실 준비 전이면 짐부터 맡기기' },
        { t:'13:00', x:'공식 체크인 시작 → 로타 홀 투어 픽업 장소·시간 재확인' },
        { t:'13:30 이후', x:'로타 홀 보트 투어 — 업체가 확정한 집합 시간과 당일 출항 판단을 우선' },
        { t:'17:30 이후', x:'피나탕 파크 가벼운 스노클 후 송송에서 저녁' },
      ],
      bring: [
        '여권 (신분증 겸용)', '경비행기 예약번호', '마스크·스노클·숏핀 (롱핀·무거운 대여 장비는 호텔에)',
        '수영복·래시가드', '아쿠아슈즈', '멀미약 (경비행기·보트)',
        '현금 넉넉히 (로타는 카드 안 되는 곳 많음)', '1박용 세면도구·여벌 옷', '모기 기피제', '방수 카메라',
      ],
      tips: [
        '사이판 공항 공영주차는 1박 최대 $20 — 주차권 사진을 찍고 차량 안에 가방·귀중품을 두지 않기',
        'Coral Garden Hotel 공식 체크인은 13:00부터 — 정오 도착 시 객실 배정 전 짐 보관을 요청',
        '로타 공항 렌터카는 항공편 도착 시각 11:00과 반납 목표 15:00를 업체에 사전 전달',
        '경비행기 짐은 전날 밤 저울로 확인 — 위탁 20lb 초과분은 $1/파운드',
        '초과·대형 짐은 다음 편으로 밀릴 수 있음 — 당일 쓸 장비·약은 휴대 짐(10lb)에',
        '롱핀·웨이트·부이는 켄싱턴 방에 — 로타 홀 투어 업체 장비로 해결',
        '도착 직후 투어 업체에 전화해 출항 여부부터 확인 — 파도 취소면 내일 오전으로 협의',
      ],
    },
    4: {
      todo: [
        { t:'07:30', x:'간단히 식사하고 Coral Garden Hotel 체크아웃 — 출발 전 바우처·결제 내역 확인' },
        { t:'08:25', x:'테테토 비치 아침 수영 — 중~만조 시간 확인, 귀중품은 보이지 않게 보관' },
        { t:'10:05', x:'스위밍 홀 — 스웰 확인, 파도가 리프를 넘으면 관망만 하고 11:20 철수' },
        { t:'11:30', x:'마른옷으로 갈아입고 점심·장비 건조, 도로·날씨 예비 시간 60분 유지' },
        { t:'14:00', x:'주유 → 로타 공항 이동, 15:00까지 렌터카 반납·항공 체크인' },
        { t:'16:30', x:'스타 마리아나스 S2 3104 출발 → 17:00 사이판 도착' },
        { t:'17:00', x:'수하물 수령·주차 정산·Enterprise 차량 회수 후 켄싱턴으로 이동' },
        { t:'저녁',  x:'18:20 전후 선셋은 지연 없을 때만, 늦어지면 호텔 디너와 휴식' },
      ...
      tips: [
        '16:30편은 일요일 마지막 사이판행 — 출발 72시간 전 강풍·결항 위험과 시간표를 재확인',
        '사이판 도착 뒤 주차권 정산·차량 외관 확인 후 켄싱턴까지 약 30~35분 운전',
        '스노클은 11:20에 끝내고 반복 딥다이빙은 생략 — 비행 전 휴식·수분 보충',
    ...
    5: {
      todo: [
        { t:'07:30', x:'조식 후 마나가하용 가벼운 장비만 챙겨 렌터카 출발' },
        { t:'08:00', x:'스마일링 코브 마리나 주차·체크인 — 세금 현금과 귀항편 시간 확인' },
        { t:'08:20', x:'첫 보트로 마나가하 이동 (약 15~20분)' },
        { t:'오전~오후', x:'숏핀·스노클로 얕은 라군 탐색 — 수영 구역과 보트 항로 경계 준수' },
        { t:'15:00', x:'막배보다 한 편 앞 보트로 스마일링 코브 복귀 후 렌터카로 호텔 이동' },
        { t:'저녁',  x:'장비 헹굼·호텔 휴식, 내일 골프 티타임·출발 시간 재확인' },
      ],
      bring: [
        '숏핀·스노클·마스크만 — 롱핀·부이·웨이트는 호텔에', '수영복·래시가드',
        '선글라스·모자', '리프 세이프 선크림', '수건·방수팩·드라이백',
        '환경세·부두세용 $13 현금 + 보트 예약 바우처', '물·간식',
      ],
      tips: [
        '마나가하는 숏핀만 가져가는 날 — 렌탈 풀세트의 롱핀·부이·웨이트는 분실 위험 때문에 호텔 보관',
        '마리나 주차 위치·보트 운영사는 전날 확정하고, 귀항은 막배 한 편 전으로 잡기',
        '바람·너울로 보트가 취소되면 Day 7 오전으로 한 번 이동하고 오늘은 리조트 휴식',
        '스노클 중에도 원업·원다운, 지정 수영 구역 밖과 선박 항로는 접근 금지',
      ],
    },
    6: {
      todo: [
      ...
      tips: [
        ...
        '클럽 대여 진행: 코럴 오션은 노캐디 · 전동카트(2인 1카트, 페어웨이 진입 가능) 기본. 그린피+카트비는 패키지 포함이지만, 클럽 세트 약 $50 + 골프백 $10(1인 1백)은 보통 별도 — 예약 확인서에 포함 여부를 미리 확인, 없으면 프로샵에서 현장 대여. 티오프 07:00~13:30, 2인부터 예약(싱글 불가)',
        '골프 어메니티 체크 항목: 18홀 그린피, 카트, 클럽/백 렌탈, 골프화 렌탈, 락커·샤워, 식음 크레딧 또는 점심 포함 여부',
        '장갑은 렌탈보다 개인 장갑 권장 — 사이즈 안 맞으면 라운딩 내내 손이 먼저 지침',
        '렌터카로 켄싱턴→코럴 오션은 약 30~35분 — 티오프 60분 전에 호텔에서 출발해 주차·체크인을 확보',
      ],
    },
    7: {
      todo: [
        { t:'오전',  x:'Day 5 마나가하 취소 시 이 시간에 재도전, 정상 진행했으면 리조트 모닝' },
        { t:'오전',  x:'{gearDays} 대여 장비를 민물로 세척·완전 건조하고 구성품·파손 여부 확인' },
        { t:'11:30', x:'렌터카로 {gearShop} 이동 → 풀세트 반납·보증금 정산' },
        { t:'낮',    x:'아이러브사이판 쇼핑 — 기념품 리스트 보고 한 번에' },
        { t:'18:00', x:'Mobil Garapan 운영시간 안에 계약서 기준 연료량까지 주유하고 영수증·게이지 사진 보관' },
        { t:'저녁',  x:'가라판 디너 또는 선셋 크루즈 — 운전자는 금주, 둘 다 마시면 차량을 두고 택시 이용' },
        { t:'23:00', x:'짐 최종 완성 — 새벽 출발이니 여권·지갑만 빼고 다 싸두기' },
      ],
      bring: [
        '대여 장비 전체·대여 계약/보증금 영수증',
        '쇼핑 리스트·에코백', '달러 현금·카드', '저녁 외출복',
        '선셋 크루즈 예약 바우처', '캐리어 여유 공간 (기념품 자리)',
      ],
      tips: [
        '대여점 영업 종료 전에 반납 시간을 확정 — 업체가 7일로 계산하면 Day 8 무인 반납 가능 여부도 협의',
        '마나가하 대체 일정이 들어오면 장비 반납·쇼핑을 오후로 미루되, 영업 마감은 넘기지 않기',
        '공식 주유소는 새벽에 닫는 곳이 많음 — 인수 때 Enterprise에 Day 7 주유 후 약 36km 운행 허용 여부나 선결제 연료 옵션을 확정',
    ...
    8: {
      todo: [
        { t:'00:15', x:'기상 — 객실 콘센트·금고·욕실 두고 가는 물건 최종 스캔' },
        { t:'00:40', x:'체크아웃 — 부대시설 이용 내역 정산 확인' },
        { t:'00:50', x:'렌터카로 공항 출발 (약 30~35분), 도착 즉시 연료 게이지·외관·주행거리 촬영' },
        { t:'01:30', x:'Enterprise 조기 반납 또는 사전 합의한 키드롭 실행 — 예약상 반납 시각은 02:30' },
        { t:'01:45', x:'항공사 체크인·수하물 위탁 — 롱핀 포장 상태 확인' },
        { t:'03:05', x:'7C3212 출발 → 06:50 인천 도착 🇰🇷' },
      ],
      bring: [
        '렌터카 계약서·주유 영수증·인수/반납 사진',
        '여권·귀국 항공권', '면세 한도 확인 (주류·담배)', '세관 신고 대상 체크',
        '기내 목베개·긴팔 (기내 냉방)', '보조배터리 기내 휴대', '지갑·휴대폰 충전기 회수',
      ],
      tips: [
        '예약상 렌터카 반납 02:30과 비행 03:05 사이는 35분뿐이라 그대로는 불가능 — 출국 전 Enterprise에 01:30 조기 반납/키드롭을 반드시 확정',
        'CDW $1,000 자기부담금·차량 반납 책임 때문에 직원 확인 또는 키드롭 절차 증빙을 남길 것',
  ...
  return { SPOTS, POIS, REFS, SHORTS, DAYS, DETAILS, GEAR_VENDOR_COORDS, TRIP, WX_FALLBACK };
})();
